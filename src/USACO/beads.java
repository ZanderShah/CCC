...
class beads
{
	public static void main(String[] args) throws IOException
	{
		...
		for (int i = 0; i < necklace.length(); i++)
		{
			...
			char rightColor = necklace.charAt(i + 1 == necklace.length() ? 0
					: i + 1);
	...
	private static Iterator<Character> reverseIterator(final int index,
	...
	private static Iterator<Character> forwardIterator(final int index,
			final String necklace)
	{
		return new Iterator<Character>() {
			int currentIndex = index;
